...
docker pull --platform linux/amd64 "$Docker_SBF_Image"
...
# macOS users on a local network can find their IP with: 
# `ifconfig | grep "inet " | grep -v 127.0.0.1`
# Then manually set DISPLAY, e.g., 
# export DISPLAY="10.0.0.212:0"
...
FROM --platform=linux/amd64 ${Docker_SBF_Image}
...
-p $NOTEBOOK_HOST_IP:$NOTEBOOK_HOST_IP \
-e SHELL=/bin/bash \
-e HOST_USER=$USER \
